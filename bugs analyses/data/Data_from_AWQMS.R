...
bugs.lim <- bugs %>%
  select(act_id, MLocID, StationDes, Project1, SampleStart_Date, Sample_Method, 
         Activity_Type, DEQ_Taxon, StageID, Result_Numeric, Result_Unit) %>%
  rename(DEQ_TAXON = DEQ_Taxon)
...
taxonomy <- taxonomy %>%
  select(DEQ_TAXON, Taxon, Phylum, Class, SubClass, Order, SubOrder, 'Super Family', Family, SubFamily, Tribe, Genus, SubGenus, SpeciesGroup, SpeciesComplex, Species, SubSpecies)
