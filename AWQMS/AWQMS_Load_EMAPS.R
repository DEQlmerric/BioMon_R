...
d_count <- counts %>% 
  ...
  filter(Project %in% c('EMAP John Day','REMAP Cascades',
                        'EMAP Statewide','EMAP Reference Condition',
                        'EMAP Reference Sites','REMAP Coastal','REMAP DEQ/QA',
                        'REMAP Deschutes')) %>%
...
write.csv(d,"//deqlab1/BioMon/Databases/RawData_to_AWQMS/EMAPS.csv",na = "",row.names = FALSE) 
