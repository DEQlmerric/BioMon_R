...
d_count <- counts %>% 
  ...
  filter(Project %in% c('Hood Basin PSP')) %>%
  ...
  mutate(colmeth = case_when(Habitat_sampled == 'R' ~ "Benthic Kick - Riffle",
         ...
         habit = "",
         Project1 = 'Pesticide Stewardship Partnerships') %>%
...
write.csv(d,"//deqlab1/BioMon/Databases/RawData_to_AWQMS/PSP.csv",na = "",row.names = FALSE) 
