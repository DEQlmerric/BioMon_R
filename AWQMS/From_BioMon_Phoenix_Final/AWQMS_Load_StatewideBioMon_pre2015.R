...
counts <- read_excel("//deqlab1/BioMon/Databases/RawData_to_AWQMS/invert_count_11Jan2021.xlsx", sheet = "Invert_count") %>%
  filter(!TAXON_CODE %in% c('DIP900601','PLE','DIP511','DIP','DIP045','ODO','DIP210415','DIP940051','CHI795', #these were determined to be 
                            'CHI9734','DIP90','TRI1631'),
         !SVN %in% c('14025UMA','14007UMA')) %>% #these were determined to be errors and couldn't be reconciled
...
d_count <- counts %>% 
  ...
  filter(Project %in% c('SE Oregon','IR Revisits','Umatilla Biomon_Tox','Ref-ODFW',
                        'Deschutes Probabilistic','PHAB Comparison','2005 Reference')) %>%
...
write.csv(d,"//deqlab1/BioMon/Databases/RawData_to_AWQMS/BioMon2.csv",na = "",row.names = FALSE)
