...
d_count <- counts %>% 
  ...
  filter(Project %in% c('BMP Effectiveness Monitoring Grass Seed Farm Streams',
                        'Alsea Basin Forestry BMP Effectiveness Study','Tickle Creek',
                        'Powder bio','Grande Ronde LT NPS','Red Boy Mine',
                        'Pine Creek','Cascade Corp','Thermofluids','Opalite Mine',
                        'Cougar Creek HWY 20','Ochoco Mine Scar Lands',
                        'Special Project','UAA','Special Project - Scio Auto',
                        'Special Project - Deep Creek')) %>%
  ...
  mutate(colmeth = case_when(Habitat_sampled == 'R' ~ "Benthic Kick - Riffle",
         ...
         habit = "",
         Project1 = 'Water Quality Response') %>%
...
write.csv(d,"//deqlab1/BioMon/Databases/RawData_to_AWQMS/WQ_res.csv",na = "",row.names = FALSE) 
